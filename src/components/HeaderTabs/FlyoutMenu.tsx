	...
	return (
		<Popover className="relative">
			<Popover.Button className="inline-flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900 group">
				...
				{isExpandable && (
					<ChevronDownIcon
						className="h-5 w-5 transition-colors group-hover:text-gray-600"
						aria-hidden="true"
					/>
				)}
			</Popover.Button>
									...
									<div className="flex gap-x-6 rounded-lg p-4 hover:bg-gray-50">
										<div className="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-gray-50">
											<InformationCircleIcon
												className="h-6 w-6 text-gray-600 group-hover:text-gray-500"
												aria-hidden="true"
											/>
										</div>
										<div>
								...
								{submenus?.map((item) => (
									<div
										...
										<div className="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
											{item.icon ? (
												<item.icon
													className="h-8 w-8 text-gray-600 group-hover:text-gray-500"
													aria-hidden="true"
												/>
											) : (
												item.image && (
